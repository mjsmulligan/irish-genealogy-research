...
def write_to_db(
    ...
    """
    Insert PlaceRow objects into place_authority. Uses INSERT OR IGNORE for
    idempotency — rows with a logainm_id already in the table are skipped.
    ...
    # Determine next available place_id
    max_id = conn.execute(
        "SELECT COALESCE(MAX(place_id), 0) FROM place_authority"
    ).fetchone()[0]
    next_id = max_id + 1
    ...
    existing_logainm_ids: set[int] = set(
        row[0] for row in conn.execute(
            "SELECT logainm_id FROM place_authority WHERE logainm_id IS NOT NULL"
        ).fetchall()
    )
    existing_manual: set[tuple] = set(
        (row[0].lower(), row[1]) for row in conn.execute(
            "SELECT name_en, place_type FROM place_authority WHERE logainm_id IS NULL"
        ).fetchall()
    )
