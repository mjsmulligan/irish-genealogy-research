...
def write_to_db(
    ...
    logainm_rows = repo.fetch_all("SELECT logainm_id FROM place_authority WHERE logainm_id IS NOT NULL")
    existing_logainm_ids: set[int] = {row['logainm_id'] for row in logainm_rows}

    manual_rows = repo.fetch_all("SELECT name_en, place_type FROM place_authority WHERE logainm_id IS NULL")
    existing_manual: set[tuple] = {(row['name_en'].lower(), row['place_type']) for row in manual_rows}
