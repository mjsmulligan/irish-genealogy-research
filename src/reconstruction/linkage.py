...
Entry point: run_census_linkage(conn, debug_log=None) -> CensusLinkageResult

Debug log
---------
Pass debug_log="path/to/linkage_debug.txt" to write a three-section log:
  ...
  SECTION 2 — SCORING DETAIL
    Surname frequency table, one row per Splink pair showing score,
    person labels, source years, surname/forename similarity, birth year
    delta, place match, and outcome band.  Proposed pairs listed separately.

  SECTION 3 — CLAUDE ANALYSIS NOTES
    Opinionated analysis of what happened: issues (thin training data, high
    null rates, surname frequency risk, score clustering near thresholds,
    suspicious merges), what looks good, and specific recommended actions.
...
class _PairRecord:
    ...
    skip_reason:      str           # e.g. "already-merged", "vanished"


@dataclass
class _DebugLog:
    """Accumulates data during pipeline execution for end-of-run log write."""
    run_ts:               str = ""
    ...
    skipped_reason:       str = ""
    # Top-N surnames by frequency, populated from feature data
...
def _merge_persons(
    ...
    # 4. relationship endpoints: re-point person_id_1 and person_id_2.
    #
    ...
    # 6. Deduplicate relationships: if re-pointing created an exact
...
def _populate_feature_stats(debug: _DebugLog, dfs: list[pd.DataFrame]) -> None:
    """Collect feature-level nullity and surname frequency from the input DataFrames."""
    if not dfs:
        return
...
def _build_label_and_source_maps(
    conn: sqlite3.Connection,
...
def _build_pair_record(
    ...
    """Extract a _PairRecord from a Splink prediction row."""
    pid_l = int(row["unique_id_l"])
    pid_r = int(row["unique_id_r"])
    score = float(row["match_probability"])

    # Feature columns retained when retain_matching_columns=True
    ...
    # Splink gamma/similarity column names vary by version — try several
...
def _write_debug_log(path: str, debug: _DebugLog, result: CensusLinkageResult) -> None:
    """Write the three-section debug log to disk."""
    out: list[str] = []
    ...
    # ── Header ─────────────────────────────────────────────────────────────
    emit(
        "GRA Linkage Pipeline — Debug Log",
    ...
    emit("", "═" * _W, "  End of GRA Linkage Debug Log", "═" * _W, "")
...
def run_census_linkage(
    ...
    """
    result = CensusLinkageResult()
    debug  = _DebugLog(run_ts=datetime.datetime.now().isoformat(timespec="seconds"))

    # ── Check active census sources ─────────────────────────────────────────
    ...
    if len(active_sources) < 2:
        ...
        if debug_log:
            _write_debug_log(debug_log, debug, result)
        return result
    ...
    _populate_feature_stats(debug, source_dfs)
    label_map, source_map = _build_label_and_source_maps(conn, source_dfs)

    # ── Splink: configure, train, predict ───────────────────────────────────
    db_api   = DuckDBAPI()
    ...
    with conn:
        for _, row in pred_df.iterrows():
            ...
            if pid_l == pid_r:
                if debug_log:
                    debug.record_pair(_build_pair_record(
