...
| 56 | **Cross-townland person linking via BMD evidence.** Test case: John McCadden (RP 25110 in 1901, age 18, son in aghlem townland place_id=2) → John McCadden (RP 26774 in 1911, age 30, head in meenadreen townland place_id=15, spouse Minnie). Son role progresses to head between censuses with ~12-year age fit. Household transitions confirm marriage + townland move. Census alone cannot link across townlands without risking false positives (unrelated cousins nearby). Solution: add marriage BMD record to bridge townland boundary. Implement person-linking layer triggered by confirmed BMD household transitions. **Note:** Deferred to BMD pipeline (item 36). Census Splink maintains strict place_id blocking to minimize false positives. | Medium (R3) | Before item 36 |
