...
| 56 | **Cross-townland person linking via BMD evidence.** Test case: John McCadden (Person #321958, RP 26774 in 1911; RP 25110 in 1901 as Person #323075). Son in aghlem townland (place_id=2) moves to meenadreen (place_id=15) after marriage to Minnie. Census alone cannot link across townlands (blocks unrelated cousins nearby). Solution: add marriage record to confirm household transition + geography change. Implement person-linking layer triggered by confirmed BMD records. **Note:** Deferred to BMD pipeline (item 36). Census Splink maintains strict place_id blocking to minimize false positives. | Medium (R3) | Before item 36 |
