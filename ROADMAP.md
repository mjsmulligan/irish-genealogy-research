...
| 12 | `repositories.md` §4: fix two stale CLI commands (`python -m src.fetch_places` → `python -m src.cli fetch-places`; `python -m src.db seed-places` → `python -m src.cli seed-places`). | ✅ Resolved (17 Jun)  |
| 13 | `validation_rules.md`: assign R-numbers to the `recorded_relationship`/`record_similarity` CHECK constraints documented but unnumbered in `database_schema.md` v3.1 §5. | ✅ Resolved (19 Jun)  |
| 14 | `conceptual_model.md` v2.6: generalised Rule 2 to an evidence-correspondence principle; resolved the Relationship evidence-FK open decision. | ✅ Resolved (19 Jun) |
...
| 19 June 2026 (session 9) | **`database_schema.md` v3.2.** Resolved Work Queue item 13: §5 Validation Rule Mapping — replaced the "pending rule assignment" bullet list with a proper table mapping R47–R50 to their DDL-level CHECK constraints on `recorded_relationship`, `record_similarity`, and `training_labels`. Note: R47–R50 had already been assigned in `validation_rules.md` §7 (v2.7 rules); this entry backfills the cross-reference in the schema doc. The `training_labels.decision` vocabulary and `person_id_1 < person_id_2` canonical-ordering constraint were found to belong to R50 (write guard) rather than requiring a new rule. |
| 19 June 2026 (session 8) | **`repositories.md` v1.6.** Resolved Work Queue item 12: fixed two stale CLI commands — `src/fetch_places.py` → `python -m src.cli fetch-places` (§1 overview and §4 Source 13 loading mechanism 1); `src/db seed-places` → `python -m src.cli seed-places` (§1 overview and §4 Source 13 loading mechanism 2). |
