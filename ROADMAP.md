...
| 12 | ~~**Hierarchical household feature for person similarity**~~ | ~~Medium~~ | 🟡 **In progress (v1.1 infrastructure staged, Splink comparison not yet active).** `household_match_score` column added to `build_census_person_features()` via `_build_household_score_lookup()`. Score computed per RecordedPerson as MAX(record_similarity.score) for their parent Record. Splink comparison levels templated but disabled: adding household context causes clustering corruption (double-linking) at current thresholds. Needs further investigation: either thresholds require system-wide recalibration or feature design should compute pair-specific scores instead of pre-aggregating. **Next steps:** (a) root-cause analysis of clustering corruption; (b) consider alternative feature computation (e.g., per-pair household match via cross-source SQL JOIN at Splink comparison time); (c) explore threshold sensitivity via EM parameter tuning. All 59 tests pass with feature infrastructure in place (comparison inactive). |
